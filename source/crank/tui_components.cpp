...
void TUIComponents::run_game_loop(TUIGameWrapper& game)
{
    ...
    auto renderer = ftxui::Renderer([&] {
        return render_game_screen(game);
    });

    // Run the event loop
    screen.Loop(renderer, on_event);

    // Clean up
    screen.Exit();
}

auto TUIComponents::render_game_screen(const TUIGameWrapper& game) -> ftxui::Element
{
    ...
    auto main_content = ftxui::vbox({
        ftxui::text("Card Crank - Terminal Game") | ftxui::bold | ftxui::center,
        ftxui::separator(),
...
auto TUIComponents::render_player_info(const GameView& view) -> ftxui::Element
{
    ...
    for (size_t i = 0; i < view.player_names.size(); ++i) {
        std::string player_info = view.player_names[i] + " (" + std::to_string(view.hand_sizes[i]) + " cards)";

        if (i == view.current_player_index) {
            player_info = "● " + player_info;  // Current player indicator
            player_elements.push_back(ftxui::text(player_info) | ftxui::bold | ftxui::color(ftxui::color::Yellow));
        } else {
...
auto TUIComponents::render_stack_display(const std::vector<Card>& stack) -> ftxui::Element
{
    if (stack.empty()) {
    ...
    card_elements.push_back(render_card(Card{'9', 'h'}, false));

    // Add other cards if any
    for (const auto& card : stack) {
        if (!(card.rank == '9' && card.suit == 'h')) {
...
auto TUIComponents::render_hand_display(const std::vector<Card>& hand, const std::vector<size_t>& selected) -> ftxui::Element
{
    if (hand.empty()) {
    ...
    for (size_t i = 0; i < hand.size(); ++i) {
        bool isSelected = std::find(selected.begin(), selected.end(), i) != selected.end();
...
auto TUIComponents::render_valid_moves(const std::vector<Play>& valid_plays) -> ftxui::Element
{
    ...
    for (size_t i = 0; i < valid_plays.size(); ++i) {
        ...
        if (valid_plays[i].indices.size() == 1) {
            valid_text += std::string(1, valid_plays[i].indices[0] + '9');
        } else {
            // For multiple cards, show count
            valid_text += std::to_string(valid_plays[i].indices.size()) + " cards";
        }
    }
...
{
    return ftxui::text("Controls: Space=Select, Enter=Play, P=Pass3, O=PassAll, Q=Quit");
}

auto TUIComponents::render_card(const Card& card, bool selected) -> ftxui::Element
{
    std::string card_text = "[" + std::string(1, card.rank) + get_card_symbol(card.suit) + "]";
...
auto TUIComponents::get_card_symbol(char suit) -> std::string
{
    switch (suit) {
        case 'h': return "♥";
        case 'd': return "♦";
...
void TUIComponents::handle_events(ftxui::ScreenInteractive& screen, TUIGameWrapper& game,
